...
export async function getLastEvents(
  ...
  const rows = await db.query(
    ...
    TRIM(to_char(event.cost, 'FM$999G999D00')) AS cost,
    TRIM(to_char(SUM(SUM(order_totals.order_total)) OVER(PARTITION BY event.event_id), 'FM$999G999D00')) AS grand_total,
    SUM(SUM(order_totals.order_total)) OVER(PARTITION BY event.event_id) - event.cost AS balance,
    TRIM(to_char(SUM(SUM(order_totals.order_total)) OVER(PARTITION BY event.event_id) - event.cost, 'FM$999G999D00')) AS balance_currency
FROM 
    event
